...
import { notifica, spedisciArretrate } from '$lib/server/notifications/service';
 ...
 *
 * **Il ritentativo delle email rimaste in coda** (ADR-0036): la tabella
 * `notifications` è anche la coda di uscita, e le righe con `emailed_at` a
 * `NULL` sono i messaggi dovuti che non sono partiti. La notte in cui una
 * chiave scaduta torna valida, partono tutte.
 *
...
export const POST: RequestHandler = async () => {
	...
	const arretrate = await spedisciArretrate(db);
