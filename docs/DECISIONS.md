...
- **Il breakpoint `md:` esiste anche in JavaScript**, in `src/lib/breakpoint.ts`, e da lì lo leggono il calendario e l'intestazione del telefono. Quasi tutto ciò che questa voce decide è CSS, e per CSS si annulla da sé riallargando la finestra; le due cose che il CSS non fa — la vista di FullCalendar, che è un'opzione JavaScript, e il blocco dello scorrimento del pannello `☰`, che è una proprietà scritta su `body` e quindi sopravvive al `display:none` dell'intestazione — vanno annullate a mano, e per farlo devono sapere quando si attraversa il confine. Il valore sta in un posto solo perché due copie dello stesso 767px si scoprono disallineate guardando un pannello che non si chiude, non leggendo un errore.
...
- **La segnalazione è un'azione e non una destinazione**, quindi non entra nell'elenco unico di navigazione di [ADR-0042](#adr-0042--sul-telefono-la-navigazione-sta-in-basso-e-il-calendario-è-un-elenco). Sopra `md:` sta nell'intestazione del calendario, accanto a "Nuova data"; sotto, in cima al pannello `☰` — **non** come quinto pulsante della barra in basso, che ADR-0042 tiene a quattro e il cui superamento è uno dei motivi per cui quella barra andrebbe ripensata. Inserire la propria data capita ogni settimana, segnalare quella di un altro molto meno.
